...
const deleted = async(id) =>{
   ...
   getAddress()
    toast("Succsessfully Address Deleted..!", {
    autoClose: 1000,
    position: toast.POSITION.TOP_RIGHT,
  });
}
...
<template>
    ...
    <section class="container">
        <div class="lg:flex md:flex max-sm:flex-none gap-5 " >
            <div  v-for="address in addresses">
                <div  class="text-left block cursor-pointer p-5 shadow max-sm:w-full rounded-lg h-auto mb-5">
                  <div class="flex justify-between ">
                    <h2 class="font-semibold text-xl mb-2  max-sm:text-sm md:text-base">OFFICE ADDRESS</h2>
                    <button @click="deleted(address.id)" class="text-red-600  font-bold top-0 right-0 w-10 h-10 ">X</button>
                  </div>
