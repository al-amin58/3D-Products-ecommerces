...
const orderCheckout = (async () =>{
  try{
    ...
    if (response.data) {  
      toast("Succsessfully Order Completed..!", {
        autoClose: 1000,
          ...
          <div class="w-5/12 max-sm:w-full md:w-full grid gap-y-5 max-sm:mt-5  ">
              <div class="max-sm:me-10  shadow max-sm:shadow-none p-3  divide-y divide-primary pb-5 rounded-lg ">
                <div class="flex justify-between  items-center mb-2 ">
                  ...
                  <div v-if="product?.bdt_price || usa_price" class="flex max-sm:text-sm md:text-sm md:ms-1">
                    <h2>{{product?.bdt_price}}</h2>
                    <span class="ps-1">৳</span>
                  </div>
                  <h2 v-else class="flex max-sm:text-sm md:text-sm md:ms-1">Free</h2>
                </div>
                <div class="flex justify-end font-semibold text-lg max-sm:text-base">
                  ...
                  <h2 v-if="product?.bdt_price || usa_price" class="text-primary flex mt-5 ms-2" >{{product?.bdt_price}} <span class="ps-1">৳</span></h2>
                  <h2 v-else class="text-primary flex mt-5 ms-2">Free</h2>
              ...
              <div class="shadow px-3 max-sm:text-sm  max-sm:me-10 py-5 max-sm:shadow-none  ">
                ...
                <div class="flex justify-between">
                 <p>Items Total</p> 
                 <p v-if="product?.bdt_price || usa_price">{{ product?.bdt_price }} <span class="ps-1">৳</span></p>
                 <p v-else >Free</p>
                </div>
                <div class="divide-y divide-primary mb-5">
                  <div class="flex justify-between ">
                    <p>Delivery Fee</p> 
                    <p>60<span class="ps-1">৳</span></p>
                  </div>
                  <div class="flex justify-between mt-5">
                    <p>Total Payment</p> 
                    <p v-if="product?.bdt_price || usa_price">{{(product?.bdt_price + 60) }}<span class="ps-1">৳</span></p>
                    <p v-else >60 $</p>
                  </div>
                </div>
